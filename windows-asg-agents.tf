...
resource "aws_cloudwatch_metric_alarm" "windows-jenkins-agents-scale-in-alarm" {
  ...
  alarm_actions     = ["${aws_autoscaling_policy.windows-jenkins-agents-scale-in-policy[0].arn}"]
}
...
resource "aws_cloudwatch_metric_alarm" "windows-jenkins-agents-scale-out-alarm" {
  ...
  alarm_actions     = ["${aws_autoscaling_policy.windows-jenkins-agents-scale-out-policy[0].arn}"]
}
...
resource "aws_launch_template" "windows-jenkins-agents" {
  ...
  user_data              = base64encode(data.template_file.windows-agents-userdata.rendered)
